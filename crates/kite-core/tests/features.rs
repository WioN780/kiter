use glam::DVec3;
use kite_core::{
    build_kite_from_def, BridleLineDef, KiteDefinition, PanelDef, SparDef, StiffJunctionDef, World,
};
...
fn t_joint_def(with_junction: bool) -> KiteDefinition {
    ...
    // with only iterations_per_substep=2 (a general solver characteristic, not specific to
    // stiff junctions — see final report).
    def.spars.push(SparDef {
